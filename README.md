...
## Next up (Phase 4 W9–W12)

1. **Skill dispatch for export (M4 continuation)** — wire `hwpx` / `docx` / `hwp-toolkit` skills behind `export_plan`'s pending entries so they transition `planned → pending → ready` automatically with proper sha256 + format-specific validation (OOXML schema, `hwpx validate`, PDF font embedding).
